...
const AdminEdit = () => {
  ...
  const _currentSlip:any = allSlips.filter((slip: any) => slip._id == query)[0];
  localStorage.setItem("currentSlip",_currentSlip)
  const currentSlip:any = localStorage.getItem("currentSlip")
  ...
  const [odds, setOdds] = useState({
    league: currentSlip?.odds?.league,
    teams: currentSlip?.odds?.teams,
    tips: currentSlip?.odds?.tips,
    result: currentSlip?.odds?.result,
  });
